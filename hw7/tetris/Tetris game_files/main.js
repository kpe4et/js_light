document.addEventListener('DOMContentLoaded', () => {
    ...
    function keyHandler(event) {
        if (gameInFocus) {
            ...
            var maxX = -1;
            gameBoard.currentFigure.forEach((cellPos) => {
                minX = Math.min(minX, (gameBoard.currentPosition+cellPos)%BOARDWIDTH);
                maxX = Math.max(maxX, (gameBoard.currentPosition+cellPos)%BOARDWIDTH);
            });
            var width = 0;
            for (var i = 0; i < gameBoard.currentFigure.length; i++) {
                for (var j = i+1; j < gameBoard.currentFigure.length; j++) {
                    width = Math.max(width, Math.abs(gameBoard.currentFigure[i]%BOARDWIDTH - gameBoard.currentFigure[j]%BOARDWIDTH));
                }
            }
            width += 1;
            console.log(minX, 'minX');
            console.log(maxX, 'maxX');
//            console.log('ширина', maxX - minX + 1); // вместо высчитывания width я мог бы использовать это значение, но есть проблемы при повороте фигур возле границ, поэтому ширина может быть неадекватной
                                            
            // console.log(event.key, event.code);
            switch (event.code) {
                case 'ArrowLeft':
                    if (minX == 0) {
                        moveFigure(gameBoard.currentPosition+BOARDWIDTH-width);
                    } else {
                ...
                case 'ArrowRight':
                    ...
                    if (maxX+1 == BOARDWIDTH) {
                        console.log('мы тут', maxX, width);
                        moveFigure(gameBoard.currentPosition-BOARDWIDTH+width);
                    } else {
                        moveFigure(gameBoard.currentPosition+1);
                    }
